...
func (decoder *addressDecoder) PrivateKeyToWIF(priv []byte, isTestnet bool) (string, error) {
	...
	cfg := maccplus_addrdec.MACC_mainnetPrivateWIFCompressed
	if decoder.wm.Config.IsTestNet {
		cfg = maccplus_addrdec.MACC_testnetPrivateWIFCompressed
	}

	wif, _ := maccplus_addrdec.Default.AddressEncode(priv, cfg)
...
func (decoder *addressDecoder) PublicKeyToAddress(pub []byte, isTestnet bool) (string, error) {
	maccplus_addrdec.Default.IsTestNet = decoder.wm.Config.IsTestNet
	address, err := maccplus_addrdec.Default.AddressEncode(pub)
	if err != nil {
		return "", err
	}
...
func (decoder *addressDecoder) RedeemScriptToAddress(pubs [][]byte, required uint64, isTestnet bool) (string, error) {
	...
	cfg := maccplus_addrdec.MACC_mainnetAddressP2SH
	if decoder.wm.Config.IsTestNet {
		cfg = maccplus_addrdec.MACC_testnetAddressP2SH
	}
	...
	address, _ := maccplus_addrdec.Default.AddressEncode(pkHash, cfg)
...
func (decoder *addressDecoder) WIFToPrivateKey(wif string, isTestnet bool) ([]byte, error) {
	...
	priv, err := maccplus_addrdec.Default.AddressDecode(wif, cfg)
	if err != nil {
		return nil, err
	}
